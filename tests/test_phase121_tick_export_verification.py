"""Phase 121 tick export verification and outlier coverage tests."""
...
def setUpModule() -> None:
    ...
    if art.is_file():
        ...
        if (
            ...
            return
    run_phase121_collection(root)


class TestPhase121Helpers(unittest.TestCase):
    def test_identity_and_prior_hashes(self) -> None:
        ...
        self.assertEqual(file_sha256(raw_dir / PHASE120_RAW_NAME), PHASE120_SHA256)
        self.assertEqual(file_sha256(raw_dir / NEW_EXPECTED_NAME), NEW_EXPECTED_SHA256)

    def test_inventory_labels(self) -> None:
        root = Path(__file__).resolve().parents[1]
        inv = {r["filename"]: r for r in inventory_raw_files(root)}
        self.assertTrue(inv[PHASE118_RAW_NAME]["is_phase118"])
        self.assertTrue(inv[PHASE120_RAW_NAME]["is_phase120"])
        self.assertTrue(inv[NEW_EXPECTED_NAME]["is_new_operator_file"])


class TestPhase121Collection(unittest.TestCase):
    def test_artifact_core(self) -> None:
        root = Path(__file__).resolve().parents[1]
        payload = json.loads((root / PHASE121_JSON).read_text(encoding="utf-8"))
        self.assertEqual(payload["PHASE121_STATUS"], "PASS")
        ...
        self.assertTrue(str(payload["NEW_EXPORT_FIRST_TICK"]).startswith("2026-01-02"))
        self.assertTrue(str(payload["NEW_EXPORT_LAST_TICK"]).startswith("2026-05-19"))
        self.assertTrue(str(payload["UNION_FIRST_TICK"]).startswith("2026-01-02"))
        self.assertTrue(str(payload["UNION_LAST_TICK"]).startswith("2026-09-07"))
        self.assertGreater(payload["UNION_ROW_COUNT"], 20_000_000)
        self.assertEqual(payload["n_events"], 419)
        self.assertGreaterEqual(payload["TICK_EVENT_COVERAGE"], 27)
        self.assertEqual(
            payload["AMBIGUOUS_394_RESOLVED_TOTAL"] + payload["AMBIGUOUS_394_REMAINING"],
            394,
        )
        self.assertGreaterEqual(payload["AMBIGUOUS_394_RESOLVED_TOTAL"], 26)
        self.assertTrue(payload["OUTLIER_31_84R_TICK_COVERAGE"])
        self.assertNotEqual(payload["OUTLIER_31_84R_CHRONOLOGY_STATUS"], "DATA_INSUFFICIENT")
    ...
    def test_docs(self) -> None:
        root = Path(__file__).resolve().parents[1]
        self.assertIn("PHASE121_STATUS", (root / PHASE121_MD).read_text(encoding="utf-8"))
        self.assertIn("H121-01", (root / LEDGER_MD).read_text(encoding="utf-8"))
        ...
        src = (root / "tradingbot/backtest/phase121_tick_export_verification.py").read_text(encoding="utf-8")
