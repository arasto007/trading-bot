"""Phase 25M — final operator MT5 evidence gate tests (offline; MT5 mocked)."""
...
from tradingbot.backtest.phase25m_run import (
    BOUNDED_ATTACH_RETRIES,
    FRESH_OPERATOR_EVIDENCE,
    PHASE25M_AUDIT_JSON,
    assess_fresh_commission,
    assess_fresh_slippage,
    build_phase25m_cost_matrix,
    run_phase25m_collection,
    run_phase25m_cost_audit,
    run_phase25m_mt5_collection,
)
...
class TestOfflinePath(unittest.TestCase):
    def test_mt5_offline_pass_with_deferral(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["No IPC connection"])
            with tempfile.TemporaryDirectory() as tmp:
                r = run_phase25m_collection(base_dir=tmp)
        self.assertEqual(r.status, "PASS_WITH_DEFERRAL")
        self.assertFalse(r.mt5_connected)

    def test_no_mt5_startup(self) -> None:
        self.assertFalse(run_phase25m_mt5_collection.__doc__ is None or "NOT start" not in (run_phase25m_mt5_collection.__doc__ or ""))

    def test_bounded_attach_deferred(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["MT5 not connected"])
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_mt5_collection(base_dir=tmp)
                raw = json.loads((Path(tmp) / "logs/phase25m_bidask_collection_raw.json").read_text())
    ...
    def test_no_symbol_select(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                r = run_phase25m_mt5_collection(base_dir=tmp)
    ...
    def test_immutability_after_on_deferred(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_mt5_collection(base_dir=tmp)
                after = Path(tmp) / "logs/phase25m_immutability_after.json"
...
class TestEvidenceClasses(unittest.TestCase):
    def test_stale_vs_fresh_separation(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["offline"])
            with tempfile.TemporaryDirectory() as tmp:
                audit = run_phase25m_cost_audit(base_dir=tmp, collection=run_phase25m_mt5_collection(base_dir=tmp))
    ...
    def test_fresh_when_connected(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog", return_value=_catalog()):
            with patch("tradingbot.backtest.phase25m_run.collect_readonly_deals_sample", return_value=([], {"ok": True})):
                with patch("tradingbot.backtest.phase25m_run.collect_historical_ticks", return_value=(None, {"ok": False})):
                    with tempfile.TemporaryDirectory() as tmp:
                        r = run_phase25m_mt5_collection(base_dir=tmp)
...
class TestCostGate(unittest.TestCase):
    ...
    def test_proxy_isolation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            _ohlc(Path(tmp))
            matrix = build_phase25m_cost_matrix(Path(tmp), None)
...
class TestDatasetCreation(unittest.TestCase):
    def test_full_path_creates_dataset(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog", return_value=_catalog()):
            with patch("tradingbot.backtest.phase25m_run.collect_readonly_deals_sample", return_value=([], {"ok": True})):
                with patch("tradingbot.backtest.phase25m_run.collect_historical_ticks") as tm:
...
class TestSafety(unittest.TestCase):
    def test_real_account_read_only(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog", return_value=_catalog("REAL")):
            with patch("tradingbot.backtest.phase25m_run.collect_readonly_deals_sample", return_value=([], {"ok": True})):
                with patch("tradingbot.backtest.phase25m_run.collect_historical_ticks", return_value=(None, {"ok": False})):
                    with tempfile.TemporaryDirectory() as tmp:
                        r = run_phase25m_mt5_collection(base_dir=tmp)
    ...
    def test_no_credentials(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["x"])
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_collection(base_dir=tmp)
                text = (Path(tmp) / PHASE25M_AUDIT_JSON).read_text().lower()
    ...
    def test_artifact_schema(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_collection(base_dir=tmp)
                data = json.loads((Path(tmp) / PHASE25M_AUDIT_JSON).read_text())
        self.assertEqual(data["phase"], "25M")
...
class TestFinalGate(unittest.TestCase):
    def test_operator_blocked_offline(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["No IPC connection"])
            with tempfile.TemporaryDirectory() as tmp:
                r = run_phase25m_collection(base_dir=tmp)
        self.assertTrue(r.operator_blocked)
        self.assertEqual(r.broker_evidence_loop_status, "BLOCKED_PENDING_OPERATOR")

    def test_bounded_attach_retries(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["offline"])
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_mt5_collection(base_dir=tmp)
                ...
                self.assertEqual(kwargs.get("legacy_config", {}).get("MT5_RETRIES"), BOUNDED_ATTACH_RETRIES)

    def test_final_gate_in_collection_report(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["x"])
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_collection(base_dir=tmp)
                data = json.loads((Path(tmp) / "logs/phase25m_collection_report.json").read_text())
        self.assertTrue(data["broker_evidence_loop_final"])
        self.assertEqual(data["phase"], "25M")

    def test_attach_attempt_metadata_offline(self) -> None:
        with patch("tradingbot.backtest.phase25m_run.collect_readonly_symbol_catalog") as m:
            m.return_value = ReadOnlyCollectionResult(ok=False, errors=["No IPC connection"])
            with tempfile.TemporaryDirectory() as tmp:
                run_phase25m_mt5_collection(base_dir=tmp)
                raw = json.loads((Path(tmp) / "logs/phase25m_bidask_collection_raw.json").read_text())
        self.assertIn("attach_attempt", raw)
        self.assertFalse(raw["attach_attempt"]["mt5_started_by_script"])
        self.assertFalse(raw["attach_attempt"]["symbol_select_called"])
        self.assertEqual(raw["attach_attempt"]["retry_count"], BOUNDED_ATTACH_RETRIES)

    def test_no_phase25n_reference(self) -> None:
        from tradingbot.backtest import phase25m_run

        text = Path(phase25m_run.__file__).read_text(encoding="utf-8")
        self.assertNotIn("phase25n", text.lower())
