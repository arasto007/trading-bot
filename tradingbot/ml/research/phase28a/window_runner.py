"""Phase 28A — window loader and dual-strategy simulation."""
...
def simulate_ab_trades(
    trades: list[dict[str, Any]],
    frame: pd.DataFrame,
) -> list[dict[str, Any]]:
    results: list[dict[str, Any]] = []
    for trade in trades:
        entry_id = str(trade.get("timestamp") or "")
        a = simulate_strategy(trade, frame, STRATEGY_A)
        b = simulate_strategy(trade, frame, STRATEGY_B)
        results.append(
            {
                "entry_id": entry_id,
...
def process_window(
    *,
    ...
    sim_results = simulate_ab_trades(trades, frame)
