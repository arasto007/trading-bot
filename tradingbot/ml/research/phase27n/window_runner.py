"""Phase 27N — window loader reusing Phase 27M replay cache."""
...
from tradingbot.ml.research.phase27n.hybrid_simulators import ALL_STRATEGIES, simulate_strategy
...
def load_window_from_phase27m_cache(
    *,
    base_dir: Path,
    phase27m_cache: Path,
    days: int,
    ...
    if not cache_records.is_file():
        return [], window, {"error": "replay_cache_missing", "days": days, "path": str(cache_records)}
...
def process_window(
    *,
    ...
    records, window, meta = load_window_from_phase27m_cache(
        base_dir=base_dir,
