#!/usr/bin/env python3
"""Phase 18C — pre-live verification (market closed, no trading)."""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from tradingbot.ml.phase18c.config import DEFAULT_SYMBOL, DEFAULT_TIMEFRAME
from tradingbot.ml.phase18c.orchestrator import run_phase18c_prelive


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Phase 18C pre-live verification")
    p.add_argument("--symbol", default=DEFAULT_SYMBOL)
    p.add_argument("--timeframe", default=DEFAULT_TIMEFRAME)
    p.add_argument("--base-dir", default=None)
    args = p.parse_args(argv)

    result = run_phase18c_prelive(
        symbol=args.symbol,
        timeframe=args.timeframe,
        base_dir=args.base_dir,
        project_root=ROOT,
    )
    print(json.dumps({
        "verdict": result["verdict"],
        "reports_dir": result["reports_dir"],
        "checks": result["checks"],
        "checklist_summary": result["checklist"].get("summary"),
    }, indent=2))
    return 0 if result["verdict"] == "READY_FOR_MARKET_OPEN" else 1


if __name__ == "__main__":
    sys.exit(main())
